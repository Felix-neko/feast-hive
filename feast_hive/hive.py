...
{#
entity_dataframe: для каждого извлекаемого entity мы прописываем следующие поля:
- его составной ключ (полный entity id)
- timestamp
- и для каждой Feature View, которая входит в entity, прописываем специальный строковый ключ
    {featureview}_entity_row_unique_id. Этот ключ собирается из строковых представлений:
    - всех entity id, входящих в это FeatureView
    - timestamp'а извлекаемого entity (не timestamp'а feature view, а именно извлекаемого entity)
#}
...
{#
Далее для каждого feature view мы создаём временную таблицу {feature_view}__outputs.
В неё мы кладём те даные, которые нам удалось извлечь из этого feature view по заданным entity.

Табличку {feature_view}__output формируем в несколько этапов.

1) {feature_view}__entity_dataframe: кусочек таблицы entity_dataframe,
но содержащий только row_unique_id только для этого feature view

2) {feature_view}__suitable_objects: ID объектов в feature view, подходящих под данное entity:
- поля составного ключа entity key для данного feature view (NB: не полный, а именно для данного feature view)
- entity_timestamp: временная метка из entity_dataframe: будем
    извлекать объекты с датой не позднее, чем эта (плюс это поле будет участвовать в конечном JOINе)
- max_event_timestamp: самый подздний event timestamp по всем найденным объектам в feature view, соответствующих
    entity key, но не превышающих заданный в entity_timestamp (и не отстающих от него больше, чем на TTL)
    Если такого объекта не найдено, то в таблице будет ещё на одну строку меньше = )

3) {feature_view}__prefinal: а теперь добавляем в предыдущую табличку с ключами и timestamp'ами найденных объектов
    ещё и собственно фичи из таблицы {feature}, от строк с заданными ключами и timestamp'ами.

    Но: у нас могут быть дубликаты (т.е. несколько строк с одинаковыми entity key и event timestamp'ами),
    для таких у нас будет row__duplicate__index > 1, записанный с помощью оконной функции ROW_NUMBER

4) {feature_view}__output: тут дубликаты мы уже зачистим
#}
...
{#
А теперь делаем жирный финальный JOIN: на строки entity_dataframe пытаемся навесить данные, найденные по ним
    в featureview'шках (если ничего подходящего не нашлось, навешиваем None, NaN или NaT)
#}
SELECT
{%- for fv in featureviews -%}
    {%- set outer_loop = loop -%}
